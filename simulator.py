...
class Car(pygame.sprite.Sprite):
	...
	def update(self):
		...
		for i in range(len(BS_sprites)):
			...
			if self.dB < calculate_DB(dis,BS_sprites[i].freq) :#and self.dB < 90:
				self.dB = calculate_DB(dis,BS_sprites[i].freq)
				self.connet_nearest = i
			if self.dB_threshold  < calculate_DB(dis_threshold,BS_sprites[i].freq) and self.dB_threshold < 90 :#and self.dB < 90:
				self.dB_threshold = calculate_DB(dis_threshold,BS_sprites[i].freq)
				self.connet_threshold = i
			if self.dB_entropy  + 15 < calculate_DB(dis_entropy,BS_sprites[i].freq) :#and self.dB < 90:
				self.dB_entropy = calculate_DB(dis_entropy,BS_sprites[i].freq)
				self.connet_entropy = i
			if self.dB_direction < calculate_DB(dis_direction,BS_sprites[i].freq) and self.dB_direction < 90 and self.rect.x > 120 and self.rect.y > 120 and self.rect.x < 480 and self.rect.y < 480:#and self.dB < 90:
